...
- Release 先用 `scripts/release-notes.mjs` 精确提取 `HISTORY.md` 的对应版本章节，再通过 `gh release create --notes-file release-notes.md --verify-tag` 发布并上传资产；版本章节缺失或为空时必须失败。不得恢复只会为直接提交生成比较链接的 `--generate-notes`，也不得恢复已弃用的 `actions/create-release` 或 `actions/upload-release-asset`。
