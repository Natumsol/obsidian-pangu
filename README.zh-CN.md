...
- 根据 Obsidian 的界面语言显示英文或简体中文命令与设置。
...
打开命令面板并执行「为中英文字符间自动加入空格」（英文界面显示为 `Add spaces between Chinese and English`），或使用默认快捷键：
...
PanGu 会在插件加载时读取 Obsidian 的界面语言。简体中文环境使用中文界面，其他语言目前回退英文。修改 Obsidian 语言后，请重新加载 PanGu。
